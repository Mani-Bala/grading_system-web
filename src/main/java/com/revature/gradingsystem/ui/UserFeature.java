	...
	public void userFeature() throws Exception {
		...
		do {
				...
				do {
					...
					try {
						...
						for (Subject subject : subjects) {
							...
							int markInt = new SubjectValidator().markValidation(subject.getName(),mark);
							sm.setMark(markInt);
			...
			case 3:
				...
				try {
					List<StudentGradeDTO> list = userFeatureService.listOfStudentService();
					System.out.println("\nGRADE    |  AVERAGE  |  REG-NO   |   STUDENT NAME");
					System.out.println("--------------------------------------------------");
					for (StudentGradeDTO studentList : list) {
						System.out.println(studentList.getGrade() + "            " + studentList.getAvg() + "        "
								+ studentList.getRegNo() + "        " + studentList.getStudentName());
					}
					System.out.println("--------------------------------------------------");
